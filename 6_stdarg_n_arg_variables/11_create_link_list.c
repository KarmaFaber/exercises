/*Enunciado del Ejercicio: Crear una Lista Enlazada Simple con Argumentos Variables

## Objetivo: Implementar una función que permita crear una lista enlazada simple utilizando una cantidad variable de argumentos, para almacenar una serie de números enteros. El número de argumentos y los valores serán pasados de manera dinámica utilizando la librería stdarg.h.

## Descripción:
En este ejercicio, implementarás una lista enlazada simple que almacene números enteros. Los números que se insertarán en la lista se proporcionarán como una cantidad variable de argumentos al llamar a la función. Para esto, usarás las funciones y macros de stdarg.h para manejar los argumentos variables.

## Especificaciones:

1. Estructura del Nodo: Define una estructura llamada Node que contenga:
 - Un campo data de tipo entero, que almacenará el valor del nodo.
 - Un puntero next de tipo Node* que apuntará al siguiente nodo en la lista enlazada.
2. Funciones a implementar:
 - Node* create_node(int value):
Crea y devuelve un nuevo nodo que almacene el valor entero pasado como argumento.
...
Crea una lista enlazada a partir de los valores enteros recibidos.
...
Crea una lista enlazada con al menos 5 números enteros pasados como argumentos.
...
int main() {
    ...
    Node* list = create_linked_list(5, 10, 20, 30, 40, 50);
